...
void genGrid(GLint N, GLint M)
{
    ...
    printf("\nKontroll pontok:\n\n");
    for (GLfloat i = 0.0; i < N; i++)
    {
        for (GLfloat j = 0.0; j < M; j++)
        {
            r = (GLfloat)(rand() % M);
...
GLfloat B(GLint n, GLint i, GLfloat u)
{
...
void genBezier(int N, int M)
{
    for (GLfloat u = 0.0f; u <= 1.0f; u += 0.1f)
    {
        for (GLfloat v = 0.0f; v <= 1.0f; v += 0.1f)
        {
            for (GLint i = 0; i < N; i++)
            {
                for (GLint j = 0; j < M; j++)
                {
                    xBezier.push_back(B(N, i, u) * B(M, j, v) * controll_vertices[i * N + j][0]);
                    yBezier.push_back(B(N, i, u) * B(M, j, v) * controll_vertices[i * N + j][1]);
                    zBezier.push_back(B(N, i, u) * B(M, j, v) * controll_vertices[i * N + j][2]);
                }
            }
...
int init()
{
    ...
    glBufferData(GL_ARRAY_BUFFER, Bezier.size() * sizeof(GLfloat), Bezier.data(), GL_STATIC_DRAW);
...
void mainRenderLoop()
{
    ...
    while (!glfwWindowShouldClose(g_window))
    {
        ...
        glBindVertexArray(vertex_array_object[0]);
        glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer_object[0]);
        glDrawArrays(GL_POINTS, 0, controll_vertices.size());
    
        glBindVertexArray(vertex_array_object[1]);
        glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer_object[1]);
        glDrawArrays(GL_POINTS, 0, Bezier.size());
